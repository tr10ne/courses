...
const AvgRatingStar = ({ value, className }) => {
  const ratingClass = `rating ${className ? `rating_${className}` : ""}`.trim();
  const ratingValueClass = `rating-value ${
    className ? `rating-value_${className}` : ""
  }`.trim();
  ...
  return (
    <div className={ratingClass}>
      ...
      <p className={ratingValueClass}>{value}</p>
    </div>
  );
};

export default AvgRatingStar;
